...
export async function generateMetadata({params}: PageProps): Promise<Metadata> {
    ...
    const value = Object.values(investObj!).sort().join(' ');

    return {
        title: `${value} | Crowd Place`,
        description: `${value} | Crowd Place`,
        alternates: {
            canonical: `${process.env.DOMAIN}/platform/${params.investmentType}/`
        }
    }
}
...
//     return investmentTypeCombinationPaths.map((url) => ({investmentType: url}))
// }
...
const Platforms = async ({searchParams, params}: PageProps) => {
    ...
    if (searchParams.page === '1') {
        redirect(`/platform/${params.investmentType}/`)
    }
