...
export async function generateMetadata({params}: PageProps): Promise<Metadata> {
    ...
    const allValue = `${valueSecondFirst} + ${valueSecondSecond} + ${valueSecondThird}`
...
async function getType(params: PageParams, page: number, perPage: number) {
    ...
    const allFilters = `${filterFirst}-${filterSecond}-${filterThird}`;

    if (investObjFirst && investObjSecond && investObjThird) {
        ...
        const queryParams = `${firstQueryParams}&and&${secondQueryParams}&and&${thirdQueryParams}`;
...
const Platforms = async ({searchParams, params}: PageProps) => {
    // if (!countryCombinationPaths.includes(params.country)) notFound();
    // if (!yearFoundedCombinationPaths.includes(params.country)) notFound();
    // if (!licenseNumberCombinationPaths.includes(params.country)) notFound();
    ...
    if (searchParams.page === '1') {
        redirect(`/platforms/${params.investmentType}/${params.industry}/${params.country}/`)
    }
