...
export async function generateMetadata({searchParams}: {searchParams: {page: string}}): Promise<Metadata> {
    ...
    const canonicalSearchParams = `${process.env.DOMAIN}/fr/blog/?page=${searchParams.page}`

    return {
        title: `Blog ${currentPage} | Crowd Place`,
        description: `Blog ${currentPage} | Crowd Place`,
        alternates: {
            canonical: !searchParams.page ? `${process.env.DOMAIN}/fr/blog/` : canonicalSearchParams
        }
    };
}
