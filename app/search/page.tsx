...
async function fetchSearchHandler(value: string, platformsPage: number = 1, postsPage: number = 1, limitPlatforms: string = '0', limitPosts: string = '0'): Promise<ISearchResult> {
    const response = await fetch(`http://1864875-cn27374.twc1.net:3001/api/search?searchParams=${value}&platformsPage=${platformsPage}&postsPage=${postsPage}&limitPlatforms=${limitPlatforms}&limitPosts=${limitPosts}`);
